...
The document named `_config` is reserved and cannot be used as a normal document see [bellow](#twins-configuration).
...
The `_config` document associates configuration values to your documents, its keys and subkeys enabling more twins features.

The `_config` document is accessed as any normal twin document, however its content has a special meaning.
...
- `_<config_key>`, `<config_value>` are configuration parameters, see [bellow](#configuration-parameters).
- `<document_key>` is a key of your document where you want a more precise configuration.

The document is seen as a tree, when a configuration parameter is set it's applied to all the subkey of this level. Setting the same `_<config_key>` again for a particular `<document_key>` will replace the values for this `<document_key>` and all it's subkey (it can be replaced by `null` to delete the configuration).

### Configuration parameters

The following parameters are available:

- `_tags`: associate a set of tags (key/value pairs) to the key.
- `_tsdb_trigger`: enable pushing all the numerical subkeys to a [Time Series Database](#time-series-database-integration). Possible values are `on-update` or `on-change`.
...
It's enabled at 2 levels:
1. in the hub's configuration set the push url of your graphite instance.
2. use the `_config` document to select which keys to push.
